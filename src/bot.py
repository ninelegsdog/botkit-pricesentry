...
async def _run_polling(state: Any, shutdown_event: asyncio.Event) -> None:
    ...
    price_check_task = asyncio.create_task(
        price_check_loop(state.bot, state.db, state.config.scheduler_interval)
    )
    try:
        await asyncio.wait([
...
async def main() -> None:
    ...
    setup_logging(level="INFO", json=True, bot_name="pricesentry")
    setup_tracing(service_name="pricesentry")
